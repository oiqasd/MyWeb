        ...
        static int GetColumnHash(IDataReader reader, int startBound = 0, int length = -1)
        {
            unchecked
            {
                ...
                for (int i = startBound; i < max; i++)
                {
                    ...
                    hash = -79 * ((hash * 31) + (tmp != null ? tmp.GetHashCode() : 0)) + (reader.GetFieldType(i) != null ? reader.GetFieldType(i).GetHashCode() : 0);
                }
                return hash;
            }
        }
...
#pragma warning disable 618
            typeof(TypeHandlerCache<>).MakeGenericType(type).GetMethod("SetHandler", BindingFlags.Static | BindingFlags.NonPublic).Invoke(null, new object[] { handler });//nameof(TypeHandlerCache<int>.SetHandler)
            if (secondary != null)
            {
...
#pragma warning restore 618
        ...
        private static GridReader QueryMultipleImpl(this IDbConnection cnn, ref CommandDefinition command)
        {
            ...
            catch
            {
                if (reader != null)
                {
                    if (!reader.IsClosed) try { if (cmd != null)cmd.Cancel(); }
...
#pragma warning restore 0618
        ...
        internal static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates, bool removeUnused, IList<LiteralToken> literals)
        {
            ...
            il.EmitCall(OpCodes.Callvirt, typeof(IDbCommand).GetProperty("Parameters").GetGetMethod(), null); //nameof(IDbCommand.Parameters)   stack is now [parameters]
            ...
            foreach (var prop in props)
            {
                if (typeof(ICustomQueryParameter).IsAssignableFrom(prop.PropertyType))
                {
                    ...
                    il.EmitCall(OpCodes.Callvirt, prop.PropertyType.GetMethod("AddParameter"), null); //nameof(ICustomQueryParameter.AddParameter)     stack is now [parameters]
                    continue;
                }
                ITypeHandler handler;
#pragma warning disable 618
...
#pragma warning restore 618
                if (dbType == DynamicParameters.EnumerableMultiParameter)
                {
                    ...
                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("PackListParameters"), null); //nameof(SqlMapper.PackListParameters)  stack is [parameters]
                    continue;
                }
                ...
                if (checkForDuplicates)
                {
                    ...
                    il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("FindOrAddParameter"), null); //nameof(SqlMapper.FindOrAddParameter)    stack is [parameters] [parameter]
                }
                else
                {
                    // no risk of duplicates; just blindly add
                    il.EmitCall(OpCodes.Callvirt, typeof(IDbCommand).GetMethod("CreateParameter"), null);//nameof(IDbCommand.CreateParameter) stack is now [parameters] [parameters] [parameter]

                    il.Emit(OpCodes.Dup);// stack is now [parameters] [parameters] [parameter] [parameter]
                    il.Emit(OpCodes.Ldstr, prop.Name); // stack is now [parameters] [parameters] [parameter] [parameter] [name]
                    il.EmitCall(OpCodes.Callvirt, typeof(IDataParameter).GetProperty("ParameterName").GetSetMethod(), null);//nameof(IDataParameter.ParameterName)  stack is now [parameters] [parameters] [parameter]
                }
                if (dbType != DbType.Time && handler == null) // https://connect.microsoft.com/VisualStudio/feedback/details/381934/sqlparameter-dbtype-dbtype-time-sets-the-parameter-to-sqldbtype-datetime-instead-of-sqldbtype-time
                {
                    ...
                    if (dbType == DbType.Object && prop.PropertyType == typeof(object)) // includes dynamic
                    {
                        ...
                        il.Emit(OpCodes.Call, typeof(SqlMapper).GetMethod("GetDbType", BindingFlags.Static | BindingFlags.Public)); //nameof(SqlMapper.GetDbType)    stack is now [parameters] [[parameters]] [parameter] [parameter] [db-type]
                    }
                    else
                    {
                    ...
                    il.EmitCall(OpCodes.Callvirt, typeof(IDataParameter).GetProperty("DbType").GetSetMethod(), null);//nameof(IDataParameter.DbType)   stack is now [parameters] [[parameters]] [parameter]
                }

                il.Emit(OpCodes.Dup);// stack is now [parameters] [[parameters]] [parameter] [parameter]
                EmitInt32(il, (int)ParameterDirection.Input);// stack is now [parameters] [[parameters]] [parameter] [parameter] [dir]
                il.EmitCall(OpCodes.Callvirt, typeof(IDataParameter).GetProperty("Direction").GetSetMethod(), null);//nameof(IDataParameter.Direction)   stack is now [parameters] [[parameters]] [parameter]
                ...
                if (prop.PropertyType.IsValueType())
                {
                    ...
                    if (callSanitize)
                    {
                        ...
                        il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("SanitizeParameterValue"), null);//nameof(SanitizeParameterValue)
                ...
                if (checkForNull)
                {
                    ...
                    il.Emit(OpCodes.Ldsfld, typeof(DBNull).GetField("Value")); // nameof(DBNull.Value)   relative stack [DBNull]
                    ...
                    if (prop.PropertyType == typeof(string))
                    {
                        ...
                        il.EmitCall(OpCodes.Callvirt, typeof(string).GetProperty("Length").GetGetMethod(), null); //nameof(string.Length)   [string] [length]
                ...
                {
#pragma warning disable 618
                    il.Emit(OpCodes.Call, typeof(TypeHandlerCache<>).MakeGenericType(prop.PropertyType).GetMethod("SetValue")); //nameof(TypeHandlerCache<int>.SetValue)   stack is now [parameters] [[parameters]] [parameter]
#pragma warning restore 618
                }
                else
                {
                    il.EmitCall(OpCodes.Callvirt, typeof(IDataParameter).GetProperty("Value").GetSetMethod(), null);//nameof(IDataParameter.Value)     stack is now [parameters] [[parameters]] [parameter]
                }
                ...
                if (prop.PropertyType == typeof(string))
                {
                    ...
                    il.EmitCall(OpCodes.Callvirt, typeof(IDbDataParameter).GetProperty("Size").GetSetMethod(), null); //nameof(IDbDataParameter.Size)    stack is now [parameters] [[parameters]] [parameter]
                ...
                else
                {
                    ...
                    // blindly add
                    il.EmitCall(OpCodes.Callvirt, typeof(IList).GetMethod("Add"), null); //nameof(IList.Add)   stack is now [parameters]
            ...
            if (literals.Count != 0 && propsArr != null)
            {
                ...
                var cmdText = typeof(IDbCommand).GetProperty("CommandText");//nameof(IDbCommand.CommandText)
...
#pragma warning restore 618
        ...
        //public static Func<Type, ITypeMap> TypeMapProvider(Type type)
        //{
        ...
        public static ITypeMap GetTypeMap(Type type)
        {
            ...
            if (map == null)
            {
                ...
                {   // double-checked; store this to avoid reflection next time we see this type
                    ...
                    if (map == null)
                    {
                        map = new DefaultTypeMap(type);// TypeMapProvider(type);
                        _typeMaps[type] = map;
                    }
                }
            }
            ...
            else
            {
                ...
                if (explicitConstr != null)
                {
                    ...
                    if (supportInitialize)
                    {
                        ...
                        il.EmitCall(OpCodes.Callvirt, typeof(ISupportInitialize).GetMethod("BeginInit"), null);//nameof(ISupportInitialize.BeginInit)
                    }
#endif
                }
                else
                ...
                if (item != null)
                {
                    ...
                    if (memberType == typeof(char) || memberType == typeof(char?))
                    {
                        il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod(
                            memberType == typeof(char) ? "ReadChar" : "ReadNullableChar", BindingFlags.Static | BindingFlags.Public), null); //nameof(SqlMapper.ReadChar) : nameof(SqlMapper.ReadNullableChar)     stack is now [target][target][typed-value]
                    }
                    else
                    {
                        ...
                        if (unboxType.IsEnum())
                        {
                            ...
                            if (colType == typeof(string))
                            {
                                ...
                                il.EmitCall(OpCodes.Call, typeof(Type).GetMethod("GetTypeFromHandle"), null);//nameof(Type.GetTypeFromHandle)    stack is now [target][target][enum-type]
                        ...
                        else
                        {
                            ...
                            if ((hasTypeHandler = typeHandlers.ContainsKey(unboxType)) || colType == unboxType || dataTypeCode == unboxTypeCode || dataTypeCode == TypeExtensions.GetTypeCode(nullUnderlyingType))
                            {
                                ...
                                {
#pragma warning disable 618
                                    il.EmitCall(OpCodes.Call, typeof(TypeHandlerCache<>).MakeGenericType(unboxType).GetMethod("Parse"), null); //nameof(TypeHandlerCache<int>.Parse)    stack is now [target][target][typed-value]
#pragma warning restore 618
                                }
                                else
                                {
            ...
            else
            {
                ...
                {
                    il.Emit(OpCodes.Ldloc_1);
                    il.EmitCall(OpCodes.Callvirt, typeof(ISupportInitialize).GetMethod("EndInit"), null);//nameof(ISupportInitialize.EndInit)
                }
#endif
            }
            il.MarkLabel(allDone);
            ...
            il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod("ThrowDataException"), null);//nameof(SqlMapper.ThrowDataException)
        ...
        private static void FlexibleConvertBoxedFromHeadOfStack(ILGenerator il, Type from, Type to, Type via)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    il.EmitCall(OpCodes.Call, typeof(Type).GetMethod("GetTypeFromHandle"), null); //nameof(Type.GetTypeFromHandle)   stack is now [target][target][value][member-type]
                    il.EmitCall(OpCodes.Call, typeof(Convert).GetMethod("ChangeType"), new Type[] { typeof(object), typeof(Type) }); // stack is now [target][target][boxed-member-type-value]
                    il.Emit(OpCodes.Unbox_Any, to); // stack is now [target][target][typed-value]
                }
            }
        }
